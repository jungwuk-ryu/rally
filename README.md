...
실제 거래나 실제 결제를 처리하지 않습니다. 모든 크레딧과 결제 화면은 시연용입니다.
...
| 변수 | 필수 | 설명 |
| --- | --- | --- |
| `PORT` | 아니요 | 서버 포트입니다. 기본값은 `8829`입니다. |
| `GEMINI_API_KEY` | 아니요 | AI MC 문구 생성에만 사용합니다. 없거나 실패해도 fallback 문구를 사용합니다. |
| `UPBIT_ACCESS_KEY` / `UPBIT_SECRET_KEY` | 아니요 | 현재 공개 ticker에는 필요하지 않습니다. 브라우저용 `VITE_` 접두사는 사용하지 않습니다. |

비밀값은 커밋하지 마세요. 공개 ticker, 준비된 MC 문구, 가상 결제 흐름만으로도 전체 데모가 작동합니다.
...
2026-08-29에 별도 로컬 프로덕션 서버(`127.0.0.1:8830`)로 다음 흐름을 확인하고 위 12장을 캡처했습니다.

- 1440×900 로비, 1600×1000 호스트 화면과 호스트 설정
- 390×844 모바일 입장 검증, 투자 전·후, 상품 선택, 선물 대상 선택, 순위
- 390×844 모바일 시연용 충전의 금액 선택, 결제 확인, 완료 상태
- 입장 → 50 크레딧 투자 → 포지션/호스트 순위 동기화 → 상품·순위 탭 → 충전 완료

캡처 과정에서 프레임워크 오류 화면과 관련 콘솔 오류가 없었고, 손님 화면의 가로 폭은 `390px`로 유지됐습니다.
